...
function initializeSocket() {
    ...
    if (typeof io === 'undefined') {
        ...
        showError('ספריית התקשורת לא נטענה. אנא וודא שהשרת פועל ורענן את הדף.');
        return;
    }
    ...
    try {
        ...
        connectionWarning.classList.remove('hidden');
        showError('שירות המשחק לא זמין. אנא וודא שהשרת פועל.');
        return;
    }
    ...
    gameState.socket.on('disconnect', () => {
        ...
        showError('התקשורת עם השרת נותקה');
    });
...
createBtn.addEventListener('click', () => {
    if (!gameState.socket || !gameState.socket.connected) {
        showError('אין חיבור לשרת. אנא וודא שהשרת פועל.');
        return;
    }
...
function updateConnectionStatus(connected) {
    if (connected) {
        ...
        showError('התקשורת עם השרת נותקה. אנא בדוק שהשרת פועל.');
    }
}
